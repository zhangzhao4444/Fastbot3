...
* **BFS agent (BFSAgent)**: Breadth-first exploration agent selectable via `--agent bfs`; layer-order exploration (one-step, then two-step, …). Implementation: [BFSAgent.cpp](./native/agent/BFSAgent.cpp). See [BFS_ALGORITHM_EXPLANATION.md](./native/agent/BFS_ALGORITHM_EXPLANATION.md).
* **DFS agent (DFSAgent)**: Depth-first exploration agent selectable via `--agent dfs`; implementation: [DFSAgent.cpp](./native/agent/DFSAgent.cpp). See [DFS_ALGORITHM_EXPLANATION.md](./native/agent/DFS_ALGORITHM_EXPLANATION.md) and [DFSAgent_algorithm_review.md](./native/agent/DFSAgent_algorithm_review.md).
...
* **Go-Explore agent (GOExploreAgent)**: Go-Explore–style exploration selectable via `--agent goexplore`; archive of UI states (cells), weighted cell selection, BFS return to target cell, then local exploration (e.g. 10 steps) from that cell. No Q-values or reuse model. Implementation: [GOExploreAgent.cpp](./native/agent/GOExploreAgent.cpp). See [GO_EXPLORE_AGENT_ALGORITHM_EXPLANATION.md](./native/agent/GO_EXPLORE_AGENT_ALGORITHM_EXPLANATION.md).
* **LLM-Explorer agent (LLMExplorerAgent)**: AIG-based knowledge-guided exploration selectable via `--agent llmexplorer` or `--agent llm_explorer`; abstract states (rule-based: activity + widget structure hash), abstract actions with exploration flags (unexplored/explored/ineffective), app-wide action selection (prefer current-screen unexplored, else random over all unexplored), and fault-tolerant navigation on the Abstract Interaction Graph. **LLM integration**: (1) **Content-aware input** — when the chosen action targets an editable widget, `getInputTextForAction()` uses the same LLM client as LLMTaskAgent (from `max.config`: `max.llm.enabled`, `max.llm.apiUrl`, etc.) to generate context-appropriate text; Model applies it to the Operate before execution. (2) **Knowledge organization** — when a new abstract state is first seen, the agent may call the LLM to group UI elements by function; the LLM returns JSON `{"groups": [[0,1],[2],...], "functions": ["desc", ...]}`. Prompts are built in Java (with a dedicated system prompt for knowledge_org to avoid format conflict); response parsing is agent-specific (C++ extracts content then parses groups/functions, with repair for common LLM JSON errors such as a missing `]`). Exploring one action in a group marks the whole group (explored/ineffective). Activity coverage efficiency depends on state abstraction and app structure (e.g. many in-page actions with the same abstract state yield mostly Ineffective steps and no cross-state AIG edges). Implementation: [LLMExplorerAgent.cpp](./native/agent/LLMExplorerAgent.cpp), [LLMExplorerAgent.h](./native/agent/LLMExplorerAgent.h). See [LLMExplorerAgent_ALGORITHM_EXPLANATION.md](./native/agent/LLMExplorerAgent_ALGORITHM_EXPLANATION.md).
