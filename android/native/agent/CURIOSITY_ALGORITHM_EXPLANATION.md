...
4. **好奇心驱动（Curiosity）**：不建路径、不建队列，每步在**当前状态**的合法动作上算 **curiosity 得分**（少访问的动作得分高、当前状态在本 episode 内被访问越多则所有动作得分被 episode 项压低、当前状态全局被访问越多则 stateFactor 放大以鼓励离开），再 **ε-greedy** 选动作。兼顾「少重复动作」「少重复状态」「离开过访状态」。

**CuriosityAgent 要解决的核心问题**：
...
### 1.2 CuriosityAgent 的定位

CuriosityAgent 是一种**无模型、基于访问计数的探索策略**：
...
在仅使用当前状态/动作计数的版本中，CuriosityAgent 无法区分「动作本身新」与「动作通常能否带来新页面」。为更快离开局部坑，当前实现为每个 `(state, action)` 近似维护一个「后继状态分布」，在得分中加入**后继状态新颖性因子**：
...
- **接入 DNN**：CuriosityAgent 创建时已默认注入 **DnnStateEncoder**（AgentFactory）。当前为**进程内 MLP**（**16→16→8**：输入=手写 16 维，ReLU+tanh，随机初始化）；setStateEncoder 时 `_clusterDim=8`。若要接入 TFLite/自训练模型：在 `DnnStateEncoder::encode()` 中在 `forwardMlp()` 处改为调用推理接口，并保持 getOutputDim() 与输出一致。
...
- **进一步优化**：可选包装器 **SelectiveStateEncoder**（运行时特征选择）、**ZScoreL2StateEncoder**（Z-score+L2 混合归一化）；**DNN** 建议离线训练或接 TFLite 使相似 UI 嵌入接近，再通过 loadWeights 或推理接口接入。
...
- **开关**：`kEnableCountSmoothing` 默认为 **true**（综合 review 建议默认开，见 §2.4.5）。常数：`kSmoothBeta`（默认 0.2，越大越紧跟原始计数）。
...
- 论文 WebRLED §3.6 使用 ε=0.4；CuriosityAgent 初始 0.4 与之一致，后期衰减到 0.05 更贪心。
...
执行完动作并进入新状态后，moveForward(nextState) 被调用。CuriosityAgent::moveForward 在调用基类**之前**保存 fromState、actionTaken，基类调用之后：
...
当 Model 在 refine/coarsen 后调用 onStateAbstractionChanged() 时，CuriosityAgent 清空：
...
| 维度           | Double SARSA        | BFS                    | FrontierAgent              | CuriosityAgent                                |
|----------------|---------------------|------------------------|----------------------------|-----------------------------------------|
...
### 5.1 Curiosity vs BFS（策略层面）

| 特性           | BFS                                    | CuriosityAgent                                          |
|----------------|----------------------------------------|---------------------------------------------------|
...
### 5.2 Curiosity vs DFS

| 特性           | DFS                    | CuriosityAgent                                  |
|----------------|------------------------|-------------------------------------------|
| **结构**       | 显式栈、trim 回溯      | 无栈；仅当前状态 + 计数                    |
| **选动作**     | 栈顶 + 未访问/reusable | 当前状态 curiosity 得分 + ε-greedy        |
| **执行**       | 纵深再回溯             | 每步独立算得分选动作，无显式路径          |

### 5.3 Curiosity vs Frontier

| 特性           | FrontierAgent                    | CuriosityAgent                                  |
|----------------|-----------------------------------|-------------------------------------------|
...
### 5.4 Curiosity vs Double SARSA

| 特性           | Double SARSA              | CuriosityAgent                    |
|----------------|----------------------------|-----------------------------|
...
CuriosityAgent 在 moveForward 与 selectNewAction 中打 BDLOG（Debug 级），便于观察算法正确性：
...
查看方式：`adb logcat -s FastbotNative:D` 或过滤包含 `CuriosityAgent:` 的日志。
...
- **使用方式**：命令行传入 **`--agent icm`** 或 **`--agent curiosity`**（大小写不敏感）。
- AgentFactory 已注册 CuriosityAgent；JNI 层将 agent 类型 "icm" 或 "curiosity" 映射到对应 AlgorithmType 即可选用 CuriosityAgent。

---

## 九、参考文献与相关文档

1. 本项目：`CuriosityAgent.cpp` / `CuriosityAgent.h`。文档：`CURIOSITY_ALGORITHM_EXPLANATION.md`。
