...
namespace fastbotx {
    ...
    ActionPtr DFSAgent::selectNewAction() {
        ...
        // Stack exhausted: last resort, use random pick on the current/new state.
        // Try to avoid BACK action to prevent infinite BACK loop when stuck at root state.
        BDLOG("DFSAgent: DFS stack exhausted, fallback to randomPickAction (preferring non-BACK)");
        
        // Try multiple times to get a non-BACK action
        ...
        for (int attempt = 0; attempt < maxAttempts; ++attempt) {
            ...
            // Last attempt or no action found, return whatever we got
            if (action) {
        ...
        // If still no action, try randomPickUnvisitedAction as last resort
        BDLOG("DFSAgent: randomPickAction returned null, trying randomPickUnvisitedAction");
        ActionPtr unvisitedAction = state->randomPickUnvisitedAction();
        if (unvisitedAction) {
            return unvisitedAction;
        }
        
        // Final fallback: return nullptr (caller should handle)
